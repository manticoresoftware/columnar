...
BlockIteratorWithSetup_i *	CreateRowidIterator ( const std::string & sAttr, Packing_e eType, uint64_t uStartOffset, uint32_t uMinRowID, uint32_t uMaxRowID, uint32_t uCount, uint32_t uRowidsPerBlock, std::shared_ptr<RD> & pSharedReader, std::shared_ptr<util::IntCodec_c> & pCodec, const common::RowidRange_t * pBounds, bool bBitmap );
