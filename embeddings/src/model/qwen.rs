...
fn dtype_from_config(config: &str) -> DType {
    ...
    match value.get("torch_dtype").and_then(Value::as_str) {
...
impl Attention {
    ...
    fn forward(&self, xs: &Tensor, attention_mask: Option<&Tensor>) -> CandleResult<Tensor> {
        ...
        if std::env::var_os("MANTICORE_QWEN_DEBUG").is_some() {
            eprintln!(
                "qwen attn: q_proj={:?} k_proj={:?} v_proj={:?} num_heads={} num_kv_heads={} head_dim={}",
                query_states.dims(),
                key_states.dims(),
                value_states.dims(),
                self.num_heads,
...
impl QwenEmbedModel {
    fn new(
        ...
        let head_dim_override = if model_info.use_pth {
            None
        } else {
            read_q_proj_out(&model_info.weights_path).and_then(|out| {
                if out > 0 && out % raw_cfg.num_attention_heads == 0 {
                    Some(out / raw_cfg.num_attention_heads)
                } else {
                    None
                }
            })
        ...
        if std::env::var_os("MANTICORE_QWEN_DEBUG").is_some() {
            ...
            eprintln!(
                "qwen debug: hidden_size={} num_heads={} num_kv_heads={} head_dim_cfg={:?} head_dim_override={:?} q_out={:?} k_out={:?}",
                raw_cfg.hidden_size,
                raw_cfg.num_attention_heads,
                raw_cfg.num_key_value_heads,
                raw_cfg.head_dim,
                head_dim_override,
                q_out,
                k_out
        ...
        let vb_m = if has_model_prefix(&model_info.weights_path) {
            vb.pp("model")
        } else {
            vb
        };
...
fn has_model_prefix(path: &Path) -> bool {
    let mut file = match std::fs::File::open(path) {
        Ok(f) => f,
        Err(_) => return true,
    };
    let mut header_len_buf = [0u8; 8];
    if file.read_exact(&mut header_len_buf).is_err() {
        return true;
    }
    ...
    let Ok(value) = serde_json::from_slice::<Value>(&header) else {
        return true;
    };
    if value.get("model.embed_tokens.weight").is_some() {
        return true;
    }
    if let Some(obj) = value.as_object() {
        return obj.keys().any(|k| k.starts_with("model."));
    }
    true
}

fn read_q_proj_out(path: &Path) -> Option<usize> {
    read_tensor_dim0(
        path,
        &[
            "layers.0.self_attn.q_proj.weight",
...
fn read_tensor_dim0(path: &Path, keys: &[&str]) -> Option<usize> {
    let mut file = std::fs::File::open(path).ok()?;
    let mut header_len_buf = [0u8; 8];
    file.read_exact(&mut header_len_buf).ok()?;
    let header_len = u64::from_le_bytes(header_len_buf) as usize;
    ...
    for key in keys {
        if let Some(shape) = value.get(*key).and_then(|v| v.get("shape")) {
            if let Some(dim0) = shape.get(0).and_then(Value::as_u64) {
